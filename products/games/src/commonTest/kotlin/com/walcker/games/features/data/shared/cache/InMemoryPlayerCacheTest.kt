...
class InMemoryPlayerCacheTest {
    ...
    private fun cache(ttlMs: Long = InMemoryPlayerCache.DEFAULT_TTL_MS) = InMemoryPlayerCache(ttlMs = ttlMs, nowMs = { now })
    ...
    @Test
    fun `returns null before anything is stored`() =
        runTest {
            val cache = cache()
    ...
    @Test
    fun `serves a stored search back`() =
        runTest {
            val cache = cache()
    ...
    @Test
    fun `different filters are different entries`() =
        runTest {
            val cache = cache()
            cache.putSearchResults(filters, results)
    ...
    @Test
    fun `entry expires once the ttl elapses`() =
        runTest {
    ...
    @Test
    fun `details expire independently`() =
        runTest {
            val cache = cache(ttlMs = 1_000L)
    ...
    @Test
    fun `invalidating a player also drops every cached search`() =
        runTest {
            val cache = cache()
            cache.putSearchResults(filters, results)
    ...
    @Test
    fun `evicts the least recently written search past the cap`() =
        runTest {
            val cache = cache()
    ...
    @Test
    fun `rewriting a key refreshes it instead of duplicating`() =
        runTest {
            val cache = cache(ttlMs = 1_000L)
            cache.putSearchResults(filters, results)
