...
class DataStoreProStateCacheTest {
    @Test
    fun `When saving Pro state should persist round trip then read returns saved value`() = runTest {
    ...
    @Test
    fun `When two users save state then each reads only its own namespace`() = runTest {
        val dataStore = PreferenceDataStoreFactory.createWithPath(
            produceFile = {
