...
exports.DEFAULT_GEOHASH_PRECISION = void 0;
exports.encodeGeohash = encodeGeohash;
exports.distanceKm = distanceKm;
exports.boundsForRadius = boundsForRadius;
const BASE32 = "0123456789bcdefghjkmnpqrstuvwxyz";
const BITS = [16, 8, 4, 2, 1];
const BITS_PER_CHAR = 5;
exports.DEFAULT_GEOHASH_PRECISION = 9;
 ...
 */
function encodeGeohash(coords, precision = exports.DEFAULT_GEOHASH_PRECISION) {
...
function boundsForRadius(center, radiusKm) {
    ...
    const ranges = [...hashes]
        .sort()
        .map((hash) => ({ start: hash, endInclusive: hash + "~" }));
    return mergeAdjacent(ranges);
}
function precisionForRadius(radiusKm) {
    let precision = exports.DEFAULT_GEOHASH_PRECISION;
    while (precision > 1) {
        if (latitudeSpanKm(precision) > radiusKm * 2)
            return precision;
        precision--;
    }
    return 1;
}
function latitudeSpanKm(precision) {
...
function kmToLongitudeDegrees(radiusKm, latitude) {
...
function mergeAdjacent(ranges) {
    ...
    for (let i = 1; i < ranges.length; i++) {
        ...
        if (current.endInclusive >= next.start) {
            current = {
                start: current.start,
                endInclusive: current.endInclusive >= next.endInclusive ? current.endInclusive : next.endInclusive,
            };
        }
        else {
            merged.push(current);
